...
def generar_imagen_cielo(datos: pd.DataFrame, sky_shape: tuple = (100, 200, 3)) -> np.ndarray:
    ...
    for _, planeta in datos.iterrows():
        # Calcular las coordenadas x e y en la matriz del cielo
        x = int((planeta['RA'] / 360) * (sky_shape[1] - 1))
        y = int((planeta['DEC'] / 180) * (sky_shape[0] - 1))
        ...
        # Asignar el color al píxel en la matriz del cielo
        cielo[y, x, :] = color
...
def filtrar_imagen_cielo(imagen: np.ndarray) -> None:
    """ Le aplica a la imagen un filtro de convolución basado en la matriz
        [[-1,-1,-1],[-1,9,-1],[-1,-1,-1]]
    Parametros:
        imagen (np.ndarray): una matriz con la imagen del cielo
    """
    # Verificar que la imagen sea una matriz de NumPy
    if not isinstance(imagen, np.ndarray):
        raise ValueError("La imagen debe ser una matriz de NumPy")
...
if __name__ == "__main__":
    ...
    deteccion_y_descubrimiento(datos, 0)  # Cambia el año según lo que necesites
    cantidad_y_tipo_deteccion(datos)
    masa_promedio_y_tipo_deteccion(datos)
    masa_planetas_vs_masa_estrellas(datos)

    # Generar y filtrar la imagen del cielo
    cielo = generar_imagen_cielo(datos)
    filtrar_imagen_cielo(cielo)
